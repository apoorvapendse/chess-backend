...
export async function createGlobalHashmap()
{
    ...
    redis.hset(`GlobalHashMap:Hashmap1`,'player1Email', 'example1@email.com', 'player2Email', 'another1@example.com', 'boardState', 'your_board_state1')
}
...
export async function addGameInRedis(email1)
{
    ...
    {
        await redis.hset(`GlobalHashMap:${email1}`,'player1Email', `${email1}`, 'player2Email', ``, 'boardState', ``,`hostColor`,``)
    }
    else{
...
export async function updateBoardState(innerHashID,boardState){
    try {
        await redis.hset(`GlobalHashMap:${innerHashID}`, 'boardState', JSON.stringify(boardState));
...
async function testFunctions() {
    ...
    await updateBoardState("apoorvavpendse@gmail.com",{ a: 12, b: "apple", c: true })

    let boardState = await redis.hget('GlobalHashMap:apoorvavpendse@gmail.com','boardState');
